...
import com.icthh.xm.gate.config.properties.TenantProperties;
...
public class TenantPropertiesService implements RefreshableConfiguration {
    ...
    public Map<String, TenantProperties.RateLimiting.RateLimitingConf> getTenantRateLimitingConfig() {
        TenantProperties tenantProperties = getTenantProps();

        if (tenantProperties != null && tenantProperties.getRateLimiting() != null
            && tenantProperties.getRateLimiting().getOauth2Client() != null) {
            return tenantProperties.getRateLimiting().getOauth2Client();
        }
