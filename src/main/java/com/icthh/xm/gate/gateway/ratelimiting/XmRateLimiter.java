...
public class XmRateLimiter extends RedisRateLimiter {
    ...
    // todo: use gateway property spring.cloud.gateway.discovery.locator.route-id-prefix
    ...
    public Map<String, Config> getConfig() {
        Map<String, Config> configMap = Optional.ofNullable(applicationProperties.getRedisRateLimiter())
            .stream()
            .flatMap(Collection::stream)
            .filter(prop -> StringUtils.isNotEmpty(prop.getRouteId()))
            .collect(Collectors
                .toMap(p -> DISCOVERY_CLIENT_PREFIX + p.getRouteId(), this::buildRateLimiterConfig)
            );

        configMap.put(DEFAULT_FILTERS_KEY, getDefaultRateLimiterConfig());
