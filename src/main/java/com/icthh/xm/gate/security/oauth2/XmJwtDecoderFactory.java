...
/**
 * Custom implementation of JwtDecoderFactory.
 * Class responsible for JWTDecoder instantiation.
 */
@Slf4j
@Getter
public class XmJwtDecoderFactory implements JwtDecoderFactory<ClientRegistration> {
    ...
    private static final Map<String, Map<String, JwtDecoder>> jwtDecoders = new ConcurrentHashMap<>();

    private Function<ClientRegistration, OAuth2TokenValidator<Jwt>> jwtValidatorFactory = new XmDefaultOidcIdTokenValidatorFactory();
    ...
    public XmJwtDecoderFactory(TenantContextHolder tenantContextHolder) {
    ...
    /**
     * Method creates and caches JWT Decoder with default claim verifiers for specific tenant and client
     *
     * @param clientRegistration Client registration object
     * @return JwtDecoder instance for specific tenant and client
     */
    @Override
    public JwtDecoder createDecoder(ClientRegistration clientRegistration) {
        ...
        return jwtDecoders.computeIfAbsent(tenantKey, key -> new HashMap<>())
            .computeIfAbsent(registrationId, clientId -> buildJwtDecoder(clientRegistration));
    }

    private NimbusJwtDecoder buildJwtDecoder(ClientRegistration clientRegistration) {
    ...
    /**
     * Method responsible for clearing stored in-memory JWT Decoders for specific tenant.
     * For example in case if client key in IDP configuration doesn't changed but clientId changed
     * in memory storage outdated claim verifiers would be present.
     * To avoid inconsistent data presence we need to clear cache for specific tenant clients when configuration updates.
     *
     * @param tenantKey tenant id
     */
    public static void clearJwtDecodersCache(String tenantKey) {
        if (CollectionUtils.isEmpty(jwtDecoders)) {
            return;
        }

        log.info("Clear jwt decoders cache for tenant [{}]", tenantKey);
        jwtDecoders.remove(tenantKey);
    }

    public static Map<String, JwtDecoder> getJwtDecoders(String tenantKey) {
        return jwtDecoders.get(tenantKey);
    }
